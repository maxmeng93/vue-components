...
解决了大数据下渲染卡顿问题，通过监听 `scroll` 事件，每次滑动到列表底部时，再追加渲染一部分数据。
组件默认支持了**可搜索**功能，不过由于 `select option` 是动态加载的，原 `el-select` 的逻辑已无法满足搜索要求，因此我自己实现了filterMethod。
